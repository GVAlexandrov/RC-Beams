...
const NewBeam2 = () => {
    ...
    useEffect(() => {
        setProjectName(beam?.projectName || '');
        setBeamLevel(beam?.beamLevel || 0);
        setbeamNumberString(beam?.beamNumberString || '');
        setRebarPosition(beam?.rebarPosition || '');
        setConcreteGrade(beam?.concrete || '');
        setAlphaCC(beam?.alphaCC || 0);
        setGammaMC(beam?.gammaMC || 0);
        setSteelGrade(beam?.steel || '');
        setGammaMS(beam?.gammaMS || 0);
        setSteelModulus(beam?.steelModulus || 0);
        setWidth(beam?.width || 0);
        setHeight(beam?.height || 0);
        setD1(beam?.d1 || 0);
        setMEd(beam?.bendingMoment || 0);
        setVEd(beam?.shearForce || 0);
        setTEd(beam?.torsion || 0);
        setRebarDiameter(beam?.rebar || 0);
    }, [beam]);
    ...
    const onSubmitHandler = (e) => {
        ...
        const rebarPositionTextError = validateNewElements.rebarPosition(rebarPosition.toLowerCase());
